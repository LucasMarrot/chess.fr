...
export default function RegisterScreen() {
    ...
    async function onSubmit(data: RegisterFormValues) {
        setLoading(true);
        const usernameToCheck = data.username.trim();

        // 1. Vérification de la disponibilité du nom d'utilisateur
        // On suppose ici que tes utilisateurs sont stockés dans une table 'profiles'
        ...
        if (checkError) {
            toast.show('Erreur', {message: 'Impossible de vérifier la disponibilité du pseudo.'});
            setLoading(false);
            return;
        }

        // 2. Si le pseudo existe déjà, on bloque l'inscription et on affiche l'erreur
        if (existingUser) {
            setError('username', {
                type: 'manual',
        ...
        // 3. Si le pseudo est libre, on procède à l'inscription standard
        const {data: authData, error} = await supabase.auth.signUp({
            email: data.email.trim().toLowerCase(),
            password: data.password,
            options: {
                data: {
                    name: usernameToCheck, // Ton username propre
                },
            },
        });
