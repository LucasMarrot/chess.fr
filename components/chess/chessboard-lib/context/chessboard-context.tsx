...
import { Arrow, BoardPosition, ChessboardProps, CustomPieces, Piece, Square } from '../types';
...
interface ChessboardProviderContext {
  ...
  boardOverlayBackdropColor: string;
  checkHighlightColor: string;
  squareOverlayTintColor: string;
  promotionModalBackgroundColor: string;
...
export const ChessboardProvider = forwardRef(
  (
    ...
    const defaults = useMemo(() => {
      const dark = String(tokens.color.dark.val);
      const light = String(tokens.color.light.val);
      const primaryDark = String(tokens.color.primaryDark.val);
      const danger = String(tokens.color.danger.val);
      const dangerDark = String(tokens.color.dangerDark.val);
      const boardWoodDark = String(tokens.color.boardWoodDark.val);
      const boardWoodLight = String(tokens.color.boardWoodLight.val);

      return {
        arrowColor: primaryDark,
        darkSquare: { backgroundColor: boardWoodDark } as Record<string, string>,
        lightSquare: { backgroundColor: boardWoodLight } as Record<string, string>,
        dropSquare: {
          boxShadow: `inset 0 0 1px 6px ${withAlpha(light, 0.75)}`,
        } as Record<string, string | number>,
        premoveDarkSquare: { backgroundColor: dangerDark } as Record<string, string | number>,
        premoveLightSquare: { backgroundColor: danger } as Record<string, string | number>,
        boardOverlayBackdropColor: withAlpha(dark, 0.7),
        checkHighlightColor: withAlpha(danger, 0.6),
        squareOverlayTintColor: withAlpha(dark, 0.1),
        promotionModalBackgroundColor: light,
      };
    }, [tokens]);

    const resolvedCustomArrowColor = customArrowColor ?? defaults.arrowColor;
    const resolvedCustomDarkSquareStyle = customDarkSquareStyle ?? defaults.darkSquare;
    const resolvedCustomDropSquareStyle = customDropSquareStyle ?? defaults.dropSquare;
    const resolvedCustomLightSquareStyle = customLightSquareStyle ?? defaults.lightSquare;
    const resolvedCustomPremoveDarkSquareStyle =
      customPremoveDarkSquareStyle ?? defaults.premoveDarkSquare;
    const resolvedCustomPremoveLightSquareStyle =
      customPremoveLightSquareStyle ?? defaults.premoveLightSquare;
    ...
    const [promoteFromSquare, setPromoteFromSquare] = useState<Square | null>(null);
    const [promoteToSquare, setPromoteToSquare] = useState<Square | null>(promotionToSquare);
    ...
    const [currentRightClickDown, setCurrentRightClickDown] = useState<Square | undefined>();
    ...
    const previousTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    ...
    const [lastSquareDraggedOver, setLastSquareDraggedOver] = useState<Square | null>(null);
    ...
    useEffect(() => {
      ...
      return () => {
        if (previousTimeoutRef.current) clearTimeout(previousTimeoutRef.current);
      };
    }, [position]);
      ...
      if (
        ...
        (arePremovesAllowed &&
          (lastPieceColour === piece[0] ||
            premovesRef.current.filter((p: Premove) => p.piece[0] === piece[0]).length > 0))
      ) {
    ...
    const ChessboardProviderContextValue: ChessboardProviderContext = {
      ...
      boardOrientation,
      boardWidth,
      boardOverlayBackdropColor: defaults.boardOverlayBackdropColor,
      chessPieces,
      checkHighlightColor: defaults.checkHighlightColor,
      clearArrows,
      clearCurrentRightClickDown,
      currentPosition,
      currentRightClickDown,
      customArrowColor: resolvedCustomArrowColor,
      customBoardStyle,
      customDarkSquareStyle: resolvedCustomDarkSquareStyle,
      customDropSquareStyle: resolvedCustomDropSquareStyle,
      customLightSquareStyle: resolvedCustomLightSquareStyle,
      customNotationStyle,
      customPremoveDarkSquareStyle: resolvedCustomPremoveDarkSquareStyle,
      customPremoveLightSquareStyle: resolvedCustomPremoveLightSquareStyle,
      customSquare,
      customSquareStyles,
      ...
      positionDifferences,
      premoves,
      promotionModalBackgroundColor: defaults.promotionModalBackgroundColor,
      promoteFromSquare,
      promoteToSquare,
      ...
      showBoardNotation,
      showPromoteDialog,
      squareOverlayTintColor: defaults.squareOverlayTintColor,
      snapToCursor,
      whiteKingInCheck,
      blackKingInCheck,
    };
