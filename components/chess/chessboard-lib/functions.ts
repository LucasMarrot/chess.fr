...
export function isDifferentFromStart(newPosition: BoardPosition): boolean {
  ...
  (Object.keys(START_POSITION_OBJECT) as (keyof typeof START_POSITION_OBJECT)[]).forEach(
    (square) => {
      if (newPosition[square] !== START_POSITION_OBJECT[square]) isDifferent = true;
    },
  );

  (Object.keys(newPosition) as (keyof typeof newPosition)[]).forEach((square) => {
...
export function getPositionDifferences(
  ...
  (Object.keys(currentPosition) as (keyof typeof currentPosition)[]).forEach((square) => {
